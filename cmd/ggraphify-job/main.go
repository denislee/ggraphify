...
func run() int {
	var (
		...
		backend = flag.String("backend", "", "LLM backend for metered commands (blank: auto-detect from an API key, else opencode, else the local claude-cli)")
		model   = flag.String("model", "", "model override (for -backend opencode-go: an OpenCode Go model id; blank keeps the one registered in ~/.graphify/providers.json)")
